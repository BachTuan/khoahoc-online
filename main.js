document.getElementById("login-res").innerHTML = '<div id="dangnhap"> <form action="" id="form-dangnhap"> <p>Bạn cần đăng nhập để truy cập khóa học, nếu chưa có tài khoản hãy bấm vào <span style="cursor:pointer;color:blue;font-weight:bold;" onclick="dangkyshow(),dangnhaphide()">ĐĂNG KÝ TÀI KHOẢN</span> </p> <div> <h2>ĐĂNG NHẬP TÀI KHOẢN</h2> </div> <input type="text" name="email" placeholder="Email" class="nhapvao" id="email-login"/><br/> <input type="password" name="password" placeholder="Mật khẩu" class="nhapvao" /><br/> <input type="checkbox" name="ghinho" id="ghinho"/><label for="ghinho">Ghi nhớ tài khoản</label><br/> <input type="submit" value="ĐĂNG NHẬP" class="nutbam"></input><input type="button" class="nutbam" value="ĐÓNG LẠI" onclick="dangnhaphide()"></input> </form> </div> <div id="dangky"> <form> <div> <h2>ĐĂNG KÝ TÀI KHOẢN</h2> </div> <input type="text" name="họ tên" placeholder="Họ Và Tên" class="nhapvao"/><br> <input type="email" name="email" placeholder="Email" class="nhapvao"/><br/> <input type="password" name="password" placeholder="Mật khẩu" class="nhapvao" /><br/> <input type="password" name="password" placeholder="Nhập lại mật khẩu" class="nhapvao" /><br/> <input type="text" name="sodienthoai" placeholder="Số điện thoại" class="nhapvao" /><br/><br/> <input type="submit" value="ĐĂNG KÝ" class="nutbam"></input><input type="button" class="nutbam" value="ĐÓNG LẠI" onclick="dangkyhide()"></input> </form>'
...
$('#form-dangnhap').on('submit', function() {
	if($('#email-login').val().trim() == "index") {
		$('#form-dangnhap').attr("action","index-login.html");
		return true;
	}
	if($('#email-login').val().trim() == "khoahoc") {
